...
def test_all_cohort_models_use_standard_source_tables() -> None:
    ...
    for model_name in ("cohort", "cohort_tier_lift", "playerscore"):
        ...
        assert 'g.game_start_dtm AS "GameStartDtm"' in queries["bets"]
        assert 'g.game_type AS "GameType"' in queries["bets"]
        ...
        assert model["parameters"]["extract_batch_size"] == 100
        assert model["parameters"]["extract_workers"] == 1
...
def test_local_cohort_window_has_all_three_model_weeks_without_output_cap() -> None:
    ...
    for model_name in ("cohort", "cohort_tier_lift", "playerscore"):
        ...
        assert parameters["extract_batch_size"] == 100
        assert parameters["extract_workers"] == 1
